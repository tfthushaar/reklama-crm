  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <p className="text-sm font-medium text-neutral-700">{fmtRange(start, end)}</p>
        <div className="ml-auto flex gap-1 rounded-full bg-neutral-100 p-1">
          {[
            ["", "All"],
            ["led", "LED"],
            ["hoarding", "Hoardings"],
            ...
            <Link
              ...
              className={cn("rounded-full px-3.5 py-1 text-[13px] font-medium transition-colors", (sp.type ?? "") === k ? "bg-white shadow-sm" : "text-neutral-600")}
            >
              {l}
            </Link>
          ))}
      ...
      <Card className="overflow-hidden">
        <div className="scrollbar-thin overflow-x-auto">
          <table className="border-separate border-spacing-0 text-xs">
            <thead>
              <tr>
                <th className="sticky left-0 z-10 min-w-56 border-b border-neutral-200 bg-white px-4 py-2 text-left font-medium text-neutral-500">Screen</th>
                {dayList.map((d) => {
                  const date = new Date(`${d}T00:00:00Z`);
                  const wk = new Intl.DateTimeFormat("en-IN", { weekday: "narrow", timeZone: "UTC" }).format(date);
                  const first = d.endsWith("-01") || d === start;
                  return (
                    <th key={d} className={cn("border-b border-neutral-200 px-0 py-1 text-center font-normal", d === t && "bg-accent-500/10")}>
                      <div className="h-3 text-[10px] font-semibold text-neutral-600">
                        {first ? new Intl.DateTimeFormat("en-IN", { month: "short", timeZone: "UTC" }).format(date) : ""}
                      </div>
                      <div className={cn("text-[11px]", d === t ? "font-bold text-accent-600" : "text-neutral-700")}>{Number(d.slice(8))}</div>
                      <div className="text-[10px] text-neutral-400">{wk}</div>
                    </th>
                  );
                })}
              </tr>
              ...
              {rows.map((a) => {
                ...
                return (
                  <tr key={a.id} className="group">
                    <td className="sticky left-0 z-10 border-b border-neutral-100 bg-white px-4 py-1.5 group-hover:bg-neutral-50">
                      <Link href={`/screens/${a.id}`} className="block text-[13px] font-medium text-neutral-800 hover:text-brand-700">
                        {a.name}
                      </Link>
                    ...
                    {dayList.map((d) => {
                      ...
                      return (
                        <td key={d} className={cn("border-b border-neutral-100 px-[1.5px] py-1.5", d === t && "bg-accent-500/10")}>
                          <div
                          ...
                          >
                            {av.capacity > 1 && v.used > 0 && v.used < av.capacity && (
                              <span className="text-[8px] leading-none font-semibold text-neutral-900">{av.capacity - v.used}</span>
                            )}
                          </div>
                        </td>
                      );
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-neutral-100 px-4 py-3">
          <Legend slots />
          <p className="text-xs text-neutral-500">Numbers show slots still free on LED screens. Hover a day for details.</p>
        </div>
      </Card>
    </div>
  );
