...
export function SendQuoteForm({
  ...
  if (state?.ok) {
    return (
      <div className="space-y-4 text-center">
        ...
        <div>
          <p className="font-semibold text-slate-900">Marked as sent</p>
          <p className="text-sm text-slate-500">The screens are held for {holdHours} hours and a follow-up reminder has been set.</p>
        </div>
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <div>
        <p className="mb-1.5 text-sm font-medium text-slate-700">How are you sending it?</p>
          ...
          {(
            ...
            <button
              ...
              className={cn(
                ...
                via === k ? "border-brand-600 bg-brand-50 text-brand-800" : "border-slate-200 text-slate-600 hover:border-slate-300",
              )}
            >
      ...
      <div className="rounded-lg bg-slate-50 p-3 text-xs whitespace-pre-line text-slate-600">{via === "email" ? mailBody : waText}</div>
      <p className="text-xs text-slate-500">After saving you&apos;ll get a button to open {via === "email" ? "your email" : "WhatsApp"} with this message ready, plus the PDF.</p>
      {state && !state.ok && <div className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-800">{state.error}</div>}
